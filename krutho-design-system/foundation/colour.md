...
This document is the Krutho Colour System. It operates under the Krutho Design Philosophy.

This document establishes:

**Primitive scales.** The raw colour ramps from which all other values derive.

**Semantic tokens.** Named values that carry intent. Each semantic token references one primitive stop and has one function.

**System architecture.** The three-layer model governing how colour operates across product, enterprise, and brand contexts.

**Governing principles.** The conditions that constrain colour application.

**Perceptual weight.** The measurement method for assessing conformance with ratio and dominance principles.
...
**Layer.** An operating context that defines the relationship between Krutho and colour in a given deployment. Three layers are defined in this document.

**Perceptual weight.** The visual prominence of a rendered element, measured by classifying it as neutral or chromatic (by OKLCH chroma) and computing its fractional surface area. The full measurement method is specified in the Perceptual Weight section.

**Dominant.** The condition in which a brand colour's chromatic area exceeds twice that of any other brand colour in the same context. Where no colour meets this threshold, no colour is dominant.

---

## System Architecture

The colour system operates across three distinct contexts. Each context has a defined relationship to colour. The three contexts are separate operating conditions with separate constraints.
...
**Layer 1: Core Technology.** The neutral system is always present. Colour is applied only as state, action, or system response. Function must remain clear and complete without colour. Function is preserved through colour failure.

**Layer 2: Enterprise Context.** Krutho is visualised within the client's world. Client colour systems may be introduced; Krutho integrates. The structural layer (surface hierarchy, text hierarchy, component behaviour) is fixed. The colour expression adapts to the client context. Krutho remains structurally consistent and visually coherent within the enterprise environment.

**Layer 3: Brand Expression.** Controlled use of Krutho brand colours. More expressive than product contexts but still restrained. Brand expression is strongest where client context is absent: investor presentations, narrative documents, and brand communications. Brand colours at this layer operate at higher density than in product or enterprise contexts, while the governing principles still apply.
...
These principles constrain how colour is applied across all three layers.

**Principle 1: Neutral first.** Every design must function completely in neutral before colour is applied. Colour is added to a working neutral system. A component, diagram, or layout that requires colour to be understood fails this condition.

**Principle 2: Colour asserts, not decorates.** Colour is applied in three conditions only: to signal state, to signal action, and to signal system response. An element carrying colour must carry a reason that traces to one of these three conditions.

**Principle 3: Single colour dominance.** Exactly one brand colour is dominant in any given context, component, or view. Dominance is defined under Perceptual Weight: a brand colour is dominant when its chromatic area exceeds twice the chromatic area of any other brand colour in the same context. Where no colour meets this threshold, no colour is dominant and this principle fails. Semantic state tokens (error, success, warning, info) are system responses, not brand colour occurrences, and are excluded from this assessment.

**Principle 4: Colour independence.** Each colour in the system is complete and independent. A colour that requires adjacent colour to produce its intended effect fails this condition.

Gradients are permitted in one defined exception: a single linear gradient between two stops of the same primitive ramp, used in illustrative diagrams to represent a continuous physical property (temperature, pressure, concentration). This exception is scoped to the diagram layer.

**Principle 5: No luminous effects.** Colour is embedded in the surface. Glow, neon, bloom, and light-emitting treatments are not admitted: these effects perform a property (energy, dynamism, modernity) that the construction does not carry.

**Principle 6: Mode-invariant brand colours.** Brand colours are identical in light and dark modes. The surface context changes, ink on light or embedded in dark, but the colour value is constant.
...
**Principle 8: System character.** Every colour decision is bounded and traceable. Every value is defined without ambiguity. Colour carries presence where it is warranted, and is withheld by default.
...
Several governing principles constrain the relationship between neutral and chromatic content. This section defines the measurement method.
...
The 2× factor is the smallest multiple at which area difference is reliably detectable by eye. A 1.5× difference reads as approximately equal at typical viewing distances.
...
OKLCH is selected over HSL because OKLCH is perceptually uniform: a fixed delta-L produces an approximately equal perceived lightness step at every position on the ramp. A ramp constructed in a non-uniform space compresses perceptually at one end and expands at the other, defeating the purpose of an evenly-spaced reference.
...
**Transitional state.** The ramp stops in this document were authored before this derivation method was specified. The hex values in the primitive scale tables do not yet conform to the OKLCH derivation defined here. Brand anchors are correct by definition; surrounding stops are approximate. Re-derivation of every primitive ramp by OKLCH stepping from its registered anchor is pending. Until re-derivation is complete, ramp stops other than anchors are documented as transitional. The neutral, signal-blue, secondary-signal, teal, amber, error, success, and warning ramps are all in scope.
...
**Why admitted.** Two independent grounds, either sufficient alone. First, diverging data requires two hues extending from a neutral midpoint with equal visual weight. Signal-blue carries the positive pole; no warm hue in the brand palette carries the negative pole. Second, four categorical diagram tokens require four perceptually distinct hue families. The three existing families (signal-blue at ~220°, secondary-signal at ~238°, teal at ~198°) are all cool. A warm hue at ~37° (amber) produces a set where each member is distinguishable from all others at diagram node sizes.
